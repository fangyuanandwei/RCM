...
class synthiaDataSet(data.Dataset):
    ...
    def __init__(
        ...
        self.data_root = '/media/lxj/d3c963a4-6608-4513-b391-9f9eb8c10a7a/DA/Data/SYNTHIA/'
        if max_iters is not None:
            self.label_to_file, self.file_to_label = pickle.load(
                open(osp.join('/media/lxj/d3c963a4-6608-4513-b391-9f9eb8c10a7a/DA/Other/proda/label_statics/synthia/', "synthia_label_info.p"), "rb"))
            self.img_ids = []
            SUB_EPOCH_SIZE = 3000
            tmp_list = []
            ind = dict()
        ...
        for name in self.img_ids:
            self.data_list.append(
                {
                    "img": os.path.join(self.data_root, "RGB/%s" % name),
                    "label": os.path.join(self.data_root, "synthia_mapped_to_cityscapes/%s" % name),
                    "name": name,
                }
            )
