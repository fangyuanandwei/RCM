...
def build_dataset(cfg, mode='train', is_source=True, epochwise=False, RCM_distance = None, Version= 'v2'):
        ...
        if is_source:
            if RCM_distance is not None:
                dataset = DatasetCatalog.get(cfg.DATASETS.SOURCE_TRAIN, mode, num_classes=cfg.MODEL.NUM_CLASSES,
                                             max_iters=iters, transform=transform, cfg=cfg, RCM_distance=RCM_distance)
            elif Version == 'v2':
                dataset = DatasetCatalog.get(cfg.DATASETS.SOURCE_TRAIN, mode, num_classes=cfg.MODEL.NUM_CLASSES,
                                             max_iters=iters, transform=transform, cfg=cfg, version='v2')
            else:
                dataset = DatasetCatalog.get(cfg.DATASETS.SOURCE_TRAIN, mode, num_classes=cfg.MODEL.NUM_CLASSES,
